...
class InfiniteInventoryVendor(Agent):
    def __init__(self, run_parameters):
        ...
        self._SINGLE_VENDOR_ID = 'Beijing Manufacturer'
    ...
    def compute_actions(self, state):
        ...
        for _, node_data in G.nodes(data=True):
            if node_data['node_type'] == 'port':
                port_locations.append(node_data['location'])

        closest_port_location = min(
                        port_locations,
                        key=lambda p: _distance(p, self._VENDOR_LOCATION))

        # Invariant: there should be 0 or 1 warehouses
        closest_port_name = next(
            ((n)
             for n, d in G.nodes(data=True)
             if d.get('location') == closest_port_location))           
